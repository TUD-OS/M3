...
impl DTU {
    ...
    fn configure(ep: EpId, lbl: Label, pe: PeId, dst_ep: EpId, crd: u64, msg_order: i32) {
    ...
    fn configure_recv(ep: EpId, buf: usize, order: i32, msg_order: i32) {
...
pub fn init() {
    ...
    DTU::configure_recv(UPCALL_REP, upc.buffer(), rbufs::UPCALL_RBUF_ORD, rbufs::UPCALL_RBUF_ORD);

    let def = RecvGate::def();
    DTU::configure_recv(DEF_REP, def.buffer(), rbufs::DEF_RBUF_ORD, rbufs::DEF_RBUF_ORD);

    thread::init();

    let eps = unsafe { EP_REGS.as_ptr() as u64 };
    syscalls::vpe_ctrl(vpe::VPE::cur().sel(), kif::syscalls::VPEOp::Init, eps).unwrap();
}
