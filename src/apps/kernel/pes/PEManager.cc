...
PEManager::PEManager()
    : _vpes(new VPE*[Platform::pe_count()]()), _count(),
...
VPE *PEManager::create(m3::String &&name, const m3::PEDesc &pe, int ep, capsel_t pfgate, bool tmuxable) {
    size_t i;
    for(i = Platform::first_pe(); i <= Platform::last_pe(); ++i) {
        if((_vpes[i] == nullptr || (tmuxable && _vpes[i]->tmuxable())) &&
